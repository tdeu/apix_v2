/**
 * Ethereum Credential Validator
 *
 * Validates Ethereum credentials by attempting to query the account balance.
 */
...
export class EthereumValidator {
  /**
   * Get the RPC URL based on credentials and network
   */
  private static getRpcUrl(credentials: ChainCredentials, network: NetworkType): string | null {
    ...
    if (credentials.infuraKey) {
      const networkName = network === 'mainnet' ? 'mainnet' : 'sepolia';
      return `https://${networkName}.infura.io/v3/${credentials.infuraKey}`;
    }

    // Check for Alchemy
    if (credentials.alchemyKey) {
      const networkName = network === 'mainnet' ? 'eth-mainnet' : 'eth-sepolia';
      return `https://${networkName}.g.alchemy.com/v2/${credentials.alchemyKey}`;
    }
  ...
  /**
   * Validate Ethereum credentials by making a test API call
  ...
  static async validate(
    ...
    if (!rpcUrl) {
      return { valid: false, error: 'No RPC provider configured (Infura, Alchemy, or custom RPC URL required)' };
    }
    ...
    try {
      ...
      if (errorMessage.includes('invalid private key')) {
        ...
        errorMessage = 'Could not connect to RPC provider - check your API key';
   ...
   * @returns True if it appears to be a valid key
   */
